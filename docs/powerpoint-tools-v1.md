...
V1 目标是先把“读 -> XML 修正/批量构图 -> 富文本 -> 验证”的闭环跑通，让模型能像 Claude PowerPoint 工具一样更自由地构建框图，同时保留现有稳定入口。
...
- `edit_slide_xml` 能通过结构化 XML 操作复刻调用链框图，文字不明显溢出，连线不飞。
...
- 原有 `connect_shapes/review_slide` 行为不回退。
