...
6. **连接器**：connect_shapes 和 create_diagram 会使用真实 PowerPoint connector 画贴边中点连接线；端点同 X/Y 时优先 Straight，否则使用 bentConnector3 肘形连接器。arrow="end" 会通过 PowerPoint 原生 tailEnd 箭头指向终点形状。不要手动画三角形箭头。
...
11. connect_shapes/create_diagram 为了修正真实连接器 XML，会进行单页 export/import，执行后 slideId 可能变化；如果 tool result 返回"新 slideId=..."，后续 review_slide/修改必须使用这个新 slideId，或不传 slideId 直接操作当前选中页。
12. 替换某个现有图形时，先调用 get_current_context（必要时带 pageNumber），确认目标形状的 slideId、shapeId 和 bounds；删除时传 slideId 或 pageNumber；随后把删除前 bounds 作为 create_diagram.canvas，确保新图落在原范围内。
13. 每次做完视觉修改（画图、添加形状、连线、create_diagram/draw_slide_shapes/edit_slide_chart/insert_icon）后，先调用 verify_slides 做程序化检查，再调用 verify_slide_visual 或 review_slide 截图检查。截图会保存到 debug-artifacts/review-slide/ 并返回路径。审查时 question 里写"请只关注最近添加的形状，忽略旧内容"。如果审查发现问题（重叠、歪斜、溢出），**必须立即修正**（用 modify_shape 移动位置、delete_shape 删除重画等），然后再审查确认。最多自查 2 轮。**不要在审查发现问题后就停止——必须修正后才能结束。**
